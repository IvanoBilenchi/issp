...
class LCG(RNG[int]):
    ...
    def __next__(self) -> int:
        # TO-DO: Update the internal state using the LCG formula and return the new value.
        return self._state
...
def compute_ac(m: int, x0: int, x1: int, x2: int) -> tuple[int, int]:
    ...
    #              exponent: x^(-1) (mod m) = pow(x, -1, m)
    #              It raises a ValueError if the inverse does not exist. Do not catch it here.
    raise ValueError
...
def find_ac(m: int, values: list[int]) -> tuple[int, int]:
    ...
    # TO-DO: Verify that the computed parameters generate the entire sequence of values.
    #        Raise a ValueError if they do not.
    raise ValueError
...
def find_lcg_params(values: list[int]) -> tuple[int, int, int]:
    # TO-DO: Create a list of candidate moduli (powers of two and Mersenne primes).
