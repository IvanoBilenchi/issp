...
from issp import common_passwords, generate_password_database, log, run_main, scrypt_fast
...
def crack_db(db: dict[int, dict[str, bytes]]) -> dict[int, str]:
    ...
    log.info("Precomputing passwords dictionary...")
    common = common_passwords()
    pwd_dict = {hash_fn(password.encode()): password for password in log.percent(common)}

    log.info("Cracking passwords...")
    for i, data in log.percent(db.items()):
